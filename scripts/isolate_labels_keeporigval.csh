...
# isolate_labels
#
# Writing out all / some labels in a binary file format -- keeping original 
# label value
...
set inputargs = ($argv);
set VERSION = 'isolate_labels_keeporigval.csh @FS_VERSION@';
...
foreach LOI ($labels)
  ...
  set cmd = (mri_binarize --i $labelvol --match $LOI --o ${outprefix}_label$LOI.nii.gz )
  set cmd = ($cmd; fslmaths ${outprefix}_label$LOI.nii.gz -mul $LOI ${outprefix}_label$LOI.origval.nii.gz)
  set cmd = ($cmd; rm -f ${outprefix}_label$LOI.nii.gz)
  echo $cmd
  eval $cmd
